...
class _Broker:
    ...
    def _close_trade(self, trade: Trade, price: float, time_index: int):
        ...
        tag = None
        if trade._sl_order:
            print(f"Price={price}, O={self._data.Open[-1]}, H={self._data.High[-1]}, L={self._data.Low[-1]}, C={self._data.Close[-1]}, PC={self._data.Close[-2]}, Time={time_index} - {trade} -- {trade._sl_order}")
...
class Backtest:
    ...
    def run(self, **kwargs) -> pd.Series:
        ...
        with np.errstate(invalid='ignore'):
            ...
            self._results["Margin_Available"] = pd.DataFrame(broker._margin_available, columns=["Margin_Available"]).set_index(data.index)
